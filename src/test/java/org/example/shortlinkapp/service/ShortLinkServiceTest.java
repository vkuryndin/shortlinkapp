 ...
 * <ul>
 *   <li>All JSON files are written into a sandbox by overriding {@code user.dir} to {@code @TempDir}.</li>
 *   <li>{@code java.awt.headless=true} ensures {@code Desktop.isDesktopSupported()} is false,
 *       so {@link ShortLinkService#openShortLink(String)} prints “Copy and open manually” and
 *       never tries to open a real browser.</li>
 *   <li>We seed {@code users.json} explicitly, because validation and ownership checks rely on it.</li>
 * </ul>
 */
public class ShortLinkServiceTest {
    ...
    @Test
    @DisplayName("openShortLink: limit reached and expired paths produce expected outputs and statuses")
    ...
    void edit_and_delete() throws Exception {
        ...
        assertTrue(stdout().contains("New limit must be >= current clicks"), "Proper message expected.");
    ...
    @Test
    @DisplayName("validateJson: reports issues for malformed links (missing fields / invalid URL / unknown owner)")
