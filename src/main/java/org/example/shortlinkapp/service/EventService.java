 ...
 * into no-ops (writers do nothing; readers return empty lists). This allows the rest of the code to
 * call the logging API unconditionally without guarding each invocation.
 *
 ...
 * ({@link #info(String, String, String)}, {@link #expired(String, String, String)}, {@link
 * #limitReached(String, String, String)}, {@link #error(String, String, String)}) set the {@link
 * EventType} for you and delegate to {@link #log(String, String, String, EventType)}.
 *
 * <p>Read helpers provide simple views:
 *
 * <ul>
 *   <li>{@link #listByOwner(String)} – all events for a given owner (in repository order)
 *   <li>{@link #recentByOwner(String, int)} – newest first, limited by {@code limit}
 *   <li>{@link #recentGlobal(int)} – newest first across all owners
 * </ul>
 */
public class EventService {
   ...
   *
   * @param enabled initially enabled state; if {@code false}, logging is disabled and the
   *     repository is not instantiated
   */
   ...
   *
   * <p>Note: If the service was constructed in a disabled state, {@link #repo} remains {@code null}
   * even after enabling. This is acceptable for the current usage pattern where toggling is used to
   * suppress writes; if you need repository-backed writes after enabling, create a new {@code
   * EventService} instance with {@code enabled=true}.
   *
   ...
   *
   * <p>If logging is disabled, returns an empty list. The {@code limit} is clamped to a minimum of
   * 1 to avoid accidental requests for zero items.
   *
