 ...
 *
 * <p>The validator is intentionally conservative: it only accepts URIs that
 * (1) parse successfully, (2) use the {@code http} or {@code https} scheme,
 * and (3) have a non-empty host. Trailing/leading whitespace is ignored.
 * Extremely long inputs are rejected early via a length check.
 *
...
public final class UrlValidator {
     ...
     * <p>Validation rules:
     * <ul>
     *   <li>Input must be non-null, non-empty after {@code trim()}, and not exceed {@code maxLen} characters.</li>
     *   <li>{@link URI#create(String)} must succeed (i.e., no {@link IllegalArgumentException}).</li>
     *   <li>Scheme must be {@code http} or {@code https} (case-insensitive).</li>
     *   <li>Host component must be present and non-blank.</li>
     * </ul>
     *
     ...
     * @return {@code true} if the input is a valid HTTP/HTTPS URL under the rules above; {@code false} otherwise
     */
