 ...
 * <ul>
 *   <li>{@link #readOrDefault(Path, Type, Object)} — read JSON into a model type, creating the
 *       file with the provided default value when it doesn't exist or when parsing returns
 *       {@code null}.</li>
 *   <li>{@link #writeAtomic(Path, Object)} — write a value as JSON to a temporary file and then atomically
 *       move it into place to prevent partial writes and reduce corruption risk.</li>
 * </ul>
 *
...
final class JsonRepository {
     ...
     * <ul>
     *   <li>Ensures the parent directory exists.</li>
     *   <li>When the file exists but JSON parsing returns {@code null}, {@code defaultValue} is returned.</li>
     *   <li>When the file does not exist, it is created and initialized with {@code defaultValue}.</li>
     * </ul>
     *
     ...
     * and then performs {@link Files#move(Path, Path, CopyOption...)} with
     * {@link StandardCopyOption#ATOMIC_MOVE} and {@link StandardCopyOption#REPLACE_EXISTING}.
     * This minimizes the risk of partially written files in case of crashes.
     *
     * <p>Preconditions and safety checks:
     * <ul>
     *   <li>{@code target} and {@code value} must be non-null.</li>
     *   <li>The parent directory must exist (it will be created if needed). If {@code target}
     *       has no parent, an {@link IOException} is thrown.</li>
     *   <li>If {@code target.getFileName()} is {@code null} (root path), a safe fallback name
     *       {@code "data"} is used for the temp file.</li>
     * </ul>
     *
     * @param target destination file to write
     * @param value the object to serialize as JSON
     * @throws IOException if the parent directory is missing and cannot be created, if the target
     *                     has no parent, or if the write/move operation fails
     */
