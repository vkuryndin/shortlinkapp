 ...
 *
 * <ul>
 *   <li>{@link #ensureCurrentUserUuid()} — loads the UUID if present; otherwise generates a new
 *       one, saves it to disk, and returns it. If persistence fails (e.g., I/O error), an ephemeral
 *       UUID is generated and returned for the current process only.
...
public final class LocalUuid {
   ...
   * @return {@code true} if the UUID was written successfully; {@code false} if the input was blank
   *     or an I/O error occurred (an error message is printed to {@code System.err})
   */
