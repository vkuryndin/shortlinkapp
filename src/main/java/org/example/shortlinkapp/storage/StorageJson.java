 ...
 *
 * <ol>
 *   <li>Serialize the content into a temp file located in the same directory as the target (e.g.,
 *       <code>.filename.tmp</code>).
 *   <li>Move the temp file over the target using {@link StandardCopyOption#ATOMIC_MOVE} and {@link
...
public final class StorageJson {
  ...
  /**
   * Writes the given list of items to the {@code target} path in JSON format using an <em>atomic
   * replace</em> strategy.
   *
   * <p>Implementation details:
   *
   * <ul>
   *   <li>Validates {@code target}, {@code list}, and {@code type} are non-null.
   *   <li>Ensures the parent directory of {@code target} exists; if the parent is {@code null}
   *       (e.g., a root path), an {@link IOException} is thrown.
   *   <li>Creates a sibling temporary file named <code>.&lt;target-filename&gt;.tmp</code> (falls
   *       back to {@code "data"} if {@code getFileName()} is {@code null}).
   *   <li>Serializes {@code list} to the temp file with UTF-8 encoding.
   *   <li>Atomically moves the temp file over {@code target} with {@link
   *       StandardCopyOption#REPLACE_EXISTING} and {@link StandardCopyOption#ATOMIC_MOVE}.
   * </ul>
   *
