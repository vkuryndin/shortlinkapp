...
// nothing special; object is in cache by reference, just flush
    ...
    public synchronized int cleanupExpiredForOwner(java.time.LocalDateTime now, String ownerUuid, boolean hardDelete) {
        int count = 0;
        if (hardDelete) {
            var it = cache.iterator();
            while (it.hasNext()) {
    ...
    public synchronized int cleanupLimitReachedForOwner(String ownerUuid, boolean hardDelete) {
        ...
        if (hardDelete) {
            ...
            while (it.hasNext()) {
                var l = it.next();
                if (ownerUuid.equals(l.ownerUuid) &&
                        l.clickLimit != null && l.clickCount >= l.clickLimit) {
                    it.remove();
                    count++;
                }
            }
