 ...
 *
 * <p>This class encapsulates all tunable parameters for the ShortlinkApp CLI application and provides
 * utilities to load them from (or persist them to) a JSON file located at
 * {@code data/config.json}. If the file is missing or unreadable, default in-memory values are
 * used and a default file is created on disk when possible.
 *
...
public class ConfigJson {
     ...
     * <p>If {@code null}, links are created with unlimited clicks unless explicitly overridden.</p>
     */
     ...
     * <ul>
     *   <li>{@code true}: remove records permanently (hard delete).</li>
     *   <li>{@code false}: only mark status (e.g., {@code EXPIRED}, {@code LIMIT_REACHED}).</li>
     * </ul>
     */
     ...
     * values and logs the cause to {@code System.err}. Parent directories are created as necessary.</p>
     *
