 ...
 *
 * <p>This repository keeps an in-memory cache that is loaded from
 * {@code data/users.json} on construction and is saved back to disk via
 * {@link JsonRepository#writeAtomic(java.nio.file.Path, Object)}.
 *
 * <p><strong>Thread-safety:</strong> This implementation is not designed for concurrent
 * mutation from multiple threads. Methods that modify the cache call {@link #flush()}
 * immediately to persist changes.
 */
public class UsersRepository {
    ...
    /**
     * Creates a repository and loads users from {@code data/users.json}. If the file does not
     * exist or cannot be read, an empty list is used and created on disk when first flushed.
     */
     ...
     * <ul>
     *   <li>If the user does not exist, a new {@link User} is created with {@code createdAt}
     *   set to {@link LocalDateTime#now()} and added to the cache.</li>
     *   <li>In all cases, {@code lastSeenAt} is refreshed to {@link LocalDateTime#now()}.</li>
     *   <li>The repository is immediately {@linkplain #flush() flushed} to disk.</li>
     * </ul>
     *
     ...
     *
     * <p>If persistence fails, the error is printed to {@code System.err} and the in-memory
