...
public class ConsoleMenu {
    ...
    private void actionCreateShortLink() {
        ...
        try {
            ...
            System.out.println("Created: " + config.baseUrl + l.shortCode + " \u2192 " + l.longUrl
                    + " | expires: " + l.expiresAt
                    + " | limit: " + (l.clickLimit == null ? "unlimited" : l.clickLimit));
    ...
    private void actionListMyLinks() {
        ...
        // --- ask filters ---
        String st = InputUtils.readTrimmed("Status filter [ALL/ACTIVE/EXPIRED/LIMIT_REACHED/DELETED] (empty=ALL): ");
        String query = InputUtils.readTrimmed("Search text (matches shortCode or URL, empty=none): ");
        String sort = InputUtils.readTrimmed("Sort [1=created desc, 2=clicks desc, 3=expires asc] (empty=1): ");

        // normalize
        String q = (query == null) ? "" : query.trim().toLowerCase();
        int sortIdx = 1;
        try { if (sort != null && !sort.isBlank()) sortIdx = Integer.parseInt(sort.trim()); } catch (NumberFormatException ignored) {}
        ...
        java.util.function.Predicate<org.example.shortlinkapp.model.ShortLink> textPred = l -> {
        ...
        switch (sortIdx) {
            case 2 -> cmp = java.util.Comparator
                    .comparingInt((org.example.shortlinkapp.model.ShortLink l) -> l.clickCount)
                    .reversed(); // clicks desc
            case 3 -> cmp = java.util.Comparator
                    .comparing((org.example.shortlinkapp.model.ShortLink l) -> l.expiresAt,
                            java.util.Comparator.nullsLast(java.util.Comparator.naturalOrder())); // expires asc (nulls last)
            default -> cmp = java.util.Comparator
                    .comparing((org.example.shortlinkapp.model.ShortLink l) -> l.createdAt,
    ...
    private void actionLinkDetails() {
        ...
        System.out.printf("%-14s %s%n", "Click Limit:",   (l.clickLimit == null ? "unlimited" : l.clickLimit));
    ...
    private void actionDeleteLink() {
        String code = InputUtils.readTrimmed("Enter short code to delete (or " + config.baseUrl + "code): ");
        if (code == null || code.isBlank()) {
            System.out.println("No code provided.");
            return;
        }
    ...
    private void actionEditClickLimit() {
        ...
        String newLimitStr = InputUtils.readTrimmed("New click limit (number >= current clicks) or 'unlimited': ");
    ...
    private void actionViewNotifications() {
        ...
        System.out.println(pad("time", 16) + " " + pad("type", 14) + " " + pad("shortCode", 10) + " message");
    ...
    private void actionReloadConfig() {
        // читаем config.json с диска, если нет — создаст дефолтный
        ...
        this.config = fresh;                       // обновляем конфиг в меню/состоянии
        this.events.setEnabled(fresh.eventsLogEnabled); // включаем/выключаем события
        this.shortLinkService.reloadConfig(fresh); // передаём новый конфиг в сервис ссылок
    ...
    private void actionListKnownUsers() {
        ...
        System.out.println(pad("#", 3) + " " + pad("UUID", 36) + " " + pad("Created", 16) + " " + pad("Last Seen", 16));
    ...
    private void actionStatistics() {
        ...
        int topN = 5;
        try { if (topStr != null && !topStr.isBlank()) topN = Math.max(1, Integer.parseInt(topStr.trim())); } catch (NumberFormatException ignored) {}
        ...
        if (isGlobal) {
            // с ownerUuid
            System.out.println(
                    pad("#", 3) + " " + pad("shortCode", 10) + " " +
                            pad("clicks", 7) + " " + pad("status", 13) + " " +
                            pad("ownerUuid", 36) + " longUrl"
            );
            ...
            // без ownerUuid (для mine)
            System.out.println(
                    pad("#", 3) + " " + pad("shortCode", 10) + " " +
                            pad("clicks", 7) + " " + pad("status", 13) + " longUrl"
            );
